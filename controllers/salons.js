...
exports.getSalons = async (req, res, next) => {
  try {
    ...
    const salons = await Salon.find(filter).populate('type city owner');
    if (!salons) {
    ...
    if (req.body.sortBy === "MostPopular")
      mappedSalons.sort((a, b) =>
        a.popularity > b.popularity ? -1 : a.popularity < b.popularity ? 1 : 0
      );
    if (req.body.sortBy === "TopRated")
      mappedSalons.sort((a, b) =>
        a.score > b.score ? -1 : a.score < b.score ? 1 : 0
      );
    if (req.body.sortBy === "QuantityOfRating")
      mappedSalons.sort((a, b) =>
        a.quantityOfRating > b.quantityOfRating
          ? -1
          : a.quantityOfRating < b.quantityOfRating
          ? 1
          : 0
    ...
    const paginateSalons = mappedSalons.slice(
      (currentPage - 1) * limit,
      currentPage * limit
    );
...
exports.freeHours = async (req, res, next) => {
  try {
    ...
    if (
      rangeStart.format("HH:mm") < startDayOfWeek.open ||
      rangeStart.format("HH:mm") > startDayOfWeek.close ||
      rangeEnd.format("HH:mm") > endDayOfWeek.close ||
      rangeEnd.format("HH:mm") < endDayOfWeek.open
    ) {
    ...
    const mappedCrew = salon.crew.map((worker) => {
      const filteredSchedule = worker.schedule.filter((task) => {
        ...
        if (
          taskStart.isSameOrAfter(rangeEnd) ||
          taskEnd.isSameOrBefore(rangeStart)
        )
          return false;
        return true;
      });
      ...
      filteredSchedule.sort((a, b) =>
        moment(a.start).isAfter(moment(b.start))
          ? 1
          : moment(b.start).isAfter(moment(a.start))
          ? -1
          : 0
      );
    ...
    mappedCrew.forEach((worker) => {
      if (!worker.schedule.length) {
        ...
        for (let i = 0; i <= worker.schedule.length; i++) {
          let end =
            worker.schedule.length > i
              ? moment(worker.schedule[i].start).utc()
              : rangeEnd.add(duration, "minutes");

          if (end.diff(start, "minutes") >= duration) {
            let range = moment.range(start, end.subtract(duration, "minutes"));
            freeTime = freeTime.concat(
              Array.from(range.by("minutes", { step: 15 }))
            );
          }

          if (i < worker.schedule.length)
            start = moment(worker.schedule[i].end).utc();
        }
        freeHours.push({
          workerId: worker.id,
          startHours: freeTime,
...
exports.confirmReservation = async (req, res, next) => {
  try {
    ...
    const checkSchedule = worker.schedule.filter((task) => {
      ...
      if (endHour.isSameOrBefore(start) || startHour.isSameOrAfter(end))
        return false;
      else return true;
    });
...
exports.addRating = async (req, res, next) => {
  try {
    ...
    if (salon.rating > 0) salon.rating = (salon.rating + score) / 2;
    else salon.rating = score;
